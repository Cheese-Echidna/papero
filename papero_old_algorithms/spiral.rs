...
pub fn create(config: Config) -> RgbaImage {
    ...
    let (mut current_x, mut current_y) = (rng.gen_range((image.width*2/5)..(image.width*3/5)), rng.gen_range((image.height/5)..(image.height*4/5)));
...
fn adjacent_avg_incl(image: &RgbaImage, x: usize, y: usize, config: Config) -> Rgba {
    let mut rng = rand::thread_rng();
...
fn average_images(images: Vec<RgbaImage>, config: Config) -> RgbaImage {
...
fn rgb_imager(red: RgbaImage, green: RgbaImage, blue: RgbaImage, config: Config) -> RgbaImage {
    let mut image: RgbaImage = RgbaImage::blank(config.width, config.height);
