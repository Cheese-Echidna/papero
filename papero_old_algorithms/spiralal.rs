...
fn adjacent_avg_incl(image: &RgbaImage, x: usize, y: usize) -> Rgba {
    ...
    // let min_mult = -5.0/MULTIPLIER as f64;
    // let max_mult = 5.0/MULTIPLIER as f64;

    let (x1,y1) = util::coords::transform_float(x,y,WIDTH,HEIGHT);
    let (x2,y2) = (x1.powi(2),y1.powi(2));

    let min_mult = -5.0/MULTIPLIER as f64/(x2+y2+1.);
    let max_mult = 5.0/MULTIPLIER as f64/(x2+y2+1.);
...
fn average_images(images: Vec<RgbaImage>) -> RgbaImage {
    let mut image: RgbaImage = RgbaImage::blank(WIDTH, HEIGHT);
...
fn rgb_imager(red: RgbaImage, green: RgbaImage, blue: RgbaImage) -> RgbaImage {
    let mut image: RgbaImage = RgbaImage::blank(WIDTH, HEIGHT);
